...
public class ModBlocksAdditions {
    ...
    public static final RegistryObject<Block> TRANSLOCATOR = registerBlock("translocator", () -> new Translocator(BlockBehaviour.Properties.copy(Blocks.IRON_BLOCK).requiresCorrectToolForDrops().strength(3.0f, 6.0f).lightLevel((state) -> state.getValue(Translocator.LIT) * 2).noOcclusion().mapColor(MapColor.METAL)));
